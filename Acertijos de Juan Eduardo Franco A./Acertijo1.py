...
RIDDLES = [
    ...
    {
        "question": "Tiene agujas pero no pincha, tiene números pero no cuenta. ¿Qué es?",
        "answer": "reloj",
    ...
    {
        "question": "Cuanto más seco, más me mojo. ¿Qué soy?",
        "answer": "toalla",
        "hint": "Te ayuda después de bañarte."
    },
    {
        "question": "No tengo boca pero hablo, no tengo piernas pero corro. ¿Qué soy?",
        "answer": "rio",
...
class RiddleGame:
    def __init__(self, master):
        ...
        self.btn_next = tk.Button(self.frame, text="Siguiente →", command=self.next_riddle, width=12)
    ...
    def countdown(self):
        self.lbl_timer.config(text=f"Tiempo: {self.timer}s")
        if self.timer <= 0:
            self.lbl_feedback.config(text="Se acabó el tiempo. Se pasa al siguiente acertijo.")
    ...
    def check_answer(self):
        ...
        if user == correct:
            ...
            self.lbl_feedback.config(text="¡Respuesta correcta! +10 pts.")
            self.answered = True
    ...
    def next_riddle(self):
        # pasar al siguiente acertijo (si el usuario nunca respondió se considera salto)
